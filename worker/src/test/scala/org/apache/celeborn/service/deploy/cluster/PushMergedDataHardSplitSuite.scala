...
class PushMergedDataHardSplitSuite extends AnyFunSuite
  ...
  override def beforeAll(): Unit = {
    ...
    val workerConf = Map(
      ...
      "celeborn.worker.shuffle.partitionSplit.max" -> "5K")

    logInfo("test initialized , setup Celeborn mini cluster")
    setUpMiniCluster(masterConf, workerConf, 2)
  }
  ...
  test("push merged data hard split") {
    ...
    // pushData until partition(0) hard_split
    val GIANT_DATA = RandomStringUtils.secure().next(1024 * 100).getBytes(StandardCharsets.UTF_8)
    shuffleClient.pushData(
      SHUFFLE_ID,
      MAP_ID,
      ATTEMPT_ID,
    ...
    assert(partitionLocationMap.get(partitions(0)).getEpoch > 0) // means partition(0) hard_split

    // push merged data, expect partition(0) hard_split, partition(1) not hard_split
    ...
    ) // means partition(1) not hard_split
  }
}
