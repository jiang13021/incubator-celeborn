...
import org.apache.celeborn.common.network.protocol.{Message, PushData, PushDataHandShake, PushMergedData, PushMergedDataUnsuccessfulPartitionInfo, RegionFinish, RegionStart, RequestMessage, RpcFailure, RpcRequest, RpcResponse, TransportMessage}
...
class PushDataHandler(val workerSource: WorkerSource) extends BaseMessageHandler with Logging {
  ...
  def handlePushMergedData(
        ...
        if (shuffleMapperAttempts.containsKey(shuffleKey)) {
          if (-1 != shuffleMapperAttempts.get(shuffleKey).get(mapId)) {
            ...
            logDebug(s"[Case1] Receive push merged data for committed hard split partition of " +
              s"(shuffle $shuffleKey, map $mapId attempt $attemptId)")
            workerSource.incCounter(WorkerSource.WRITE_DATA_HARD_SPLIT_COUNT)
            pushMergedDataCallback.addUnsuccessfulPartition(index, StatusCode.HARD_SPLIT)
          }
        } else {
          if (storageManager.shuffleKeySet().contains(shuffleKey)) {
            // If there is no shuffle key in shuffleMapperAttempts but there is shuffle key
            ...
            pushMergedDataCallback.addUnsuccessfulPartition(
              index,
      ...
      case (fileWriter, index) =>
        if (fileWriter == null) {
          ...
          if (splitStatus == StatusCode.HARD_SPLIT) {
            ...
            pushMergedDataCallback.addUnsuccessfulPartition(index, StatusCode.SOFT_SPLIT)
          }
        }
        if (!pushMergedDataCallback.containIndex(index) || pushMergedDataCallback.getStatusCode(
    ...
    if (doReplicate) {
            ...
            override def onSuccess(response: ByteBuffer): Unit = {
              ...
              try {
                val pushMergedDataResponse =
                  Message.decode(response).asInstanceOf[PushMergedDataUnsuccessfulPartitionInfo]
                pushMergedDataCallback.unionUnsuccessfulPartition(
                  pushMergedDataResponse.unsuccessfulPartitionIndexes,
                  pushMergedDataResponse.statusCodes)
              } catch {
      ...
      writeLocalData(
        ...
        Some(batchOffsets),
        writePromise,
        pushMergedDataCallback.getSortedSkipIndexes())
    } else {
      // The codes here could be executed if
      // 1. the client doesn't enable push data to the replica, the primary worker could hit here
      // 2. the client enables push data to the replica, and the replica worker could hit here
  ...
  class PushMergedDataCallback(callback: RpcResponseCallback) {
    ...
    def getSortedSkipIndexes(): Array[Int] = {
      partitionIndex2StatusCode.collect {
    ...
    def onSuccess(status: StatusCode): Unit = {
      val length = partitionIndex2StatusCode.size
      val unsuccessfulPartitionIndexes = new Array[Int](length)
      val statusCodes = new Array[Byte](length)
      var i = 0
      partitionIndex2StatusCode.foreach {
        case (partitionIndex, statusCode) =>
          unsuccessfulPartitionIndexes(i) = partitionIndex
          statusCodes(i) = statusCode
          i += 1
      }
      val reason: Byte =
        if (partitionIndex2StatusCode.isEmpty || status == StatusCode.MAP_ENDED) {
          status.getValue
        } else {
          StatusCode.HARD_SPLIT.getValue
        }
      val pushMergedDataInfo =
        new PushMergedDataUnsuccessfulPartitionInfo(unsuccessfulPartitionIndexes, statusCodes)
      val pushMergedDataInfoByteBuffer = pushMergedDataInfo.toByteBuffer
  ...
  private def writeLocalData(
      ...
      sortedSkipIndexes: Array[Int] = Array.empty[Int]): Unit = {
    ...
    batchOffsets match {
      case Some(batchOffsets) =>
        ...
        while (index < fileWriters.length) {
          if (indexOfSkipIndexes < skipListLength && index == sortedSkipIndexes(
              indexOfSkipIndexes)) {
